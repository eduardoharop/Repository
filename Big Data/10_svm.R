...
svm = ksvm(letter ~ ., data = data_train, kernel = "vanilladot")
svm_P = predict(svm, data_test)

table(svm_P, data_test$letter)

correct = (svm_P == data_test$letter)
table(correct)
prop.table(table(correct))

svm = ksvm(letter ~ ., data = data_train, kernel = "rbfdot")
svm_P = predict(svm, data_test)
